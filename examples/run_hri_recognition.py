...
from sense.downstream_tasks.hri_recognition import INT2LAB
from sense.downstream_tasks.hri_recognition import GESTURE_THRESHOLDS
...
if __name__ == "__main__":
    ...
    # Load custom network
    checkpoint_classifier = load_weights_from_resources('hri_recognition/fyp5024.checkpoint')
    ...
    hri_classifier = LogisticRegression(num_in=backbone_network.feature_dim,
                                           num_out=len(INT2LAB))
    hri_classifier.load_state_dict(checkpoint_classifier)
    hri_classifier.eval()

    # Concatenate backbone network and logistic regression
    net = Pipe(backbone_network, hri_classifier)
